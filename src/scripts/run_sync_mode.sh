...
# VIDEOPATH=$HOME/Desktop/C3OD/data/$DATASET/images
#
...
if [ $gf_type -eq 1 ]
then
  RESPATH=$HOME/Desktop/AVIS/Projects/C3OD/results/${DATASET}_${v1}vs${v2}/netvlad
  # RESPATH=$HOME/Desktop/C3OD/results/${DATASET}_${v1}vs${v2}/netvlad/FREQ${FEAT_SHARING_FREQ}
  VPRCALLPATH=$HOME/Desktop/C3OD/feat/${DATASET}_${v1}vs${v2}/BTST/run$R
elif [ $gf_type -eq 2 ]
then 
  RESPATH=$HOME/Desktop/AVIS/Projects/C3OD/results/${DATASET}_${v1}vs${v2}/deepbit
  # RESPATH=$HOME/Desktop/C3OD/results/${DATASET}_${v1}vs${v2}/deepbit/FREQ${FEAT_SHARING_FREQ}
  VPRCALLPATH=$HOME/Desktop/C3OD/feat/${DATASET}_${v1}vs${v2}/BTST/run$R
else
  RESPATH=$HOME/Desktop/AVIS/Projects/C3OD/results/${DATASET}_${v1}vs${v2}/dbow
  # RESPATH=$HOME/Desktop/C3OD/results/${DATASET}_${v1}vs${v2}/dbow/FREQ${FEAT_SHARING_FREQ}
  VPRCALLPATH=$HOME/Desktop/C3OD/feat/${DATASET}_${v1}vs${v2}/BTST/run$R
fi
...
./Agent 1 ${ID1[j]} ${ID2[j]} $SEQ1 $RESPATH $ORBVOC $num_max_frames $max_num_kps $matching_th $b_snn $snn_th $gf_type $gf_dim $FEATPATH1 $COLLAB_MODE $INIT_WND $FEAT_SHARING_FREQ $SYNC_MODE $VPRCALLFILE1 $FREQ 2> ${RESPATH}/log_agent1_${v1}vs${v2}_run$R.txt & 

./Agent 2 ${ID2[j]} ${ID1[j]} $SEQ2 $RESPATH $ORBVOC $num_max_frames $max_num_kps $matching_th $b_snn $snn_th $gf_type $gf_dim $FEATPATH2 $COLLAB_MODE $INIT_WND $FEAT_SHARING_FREQ $SYNC_MODE $VPRCALLFILE2 $FREQ 2> ${RESPATH}/log_agent2_${v1}vs${v2}_run$R.txt &

wait

zip run$R.zip *agent{1,2}*
rm *agent{1,2}*
